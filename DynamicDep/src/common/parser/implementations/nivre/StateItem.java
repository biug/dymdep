...
public class StateItem extends StateItemBase {
	...
	protected int buffer_back;
	protected int[] m_lBuffer;
	...
	public StateItem() {
		...
		buffer_back = -1;
		m_lBuffer = new int[Macros.MAX_SENTENCE_SIZE];
	...
	public final int buffertop() {
		if (buffer_back != -1) {
			return m_lBuffer[buffer_back];
		}
		return m_nNextWord;
	}
	...
	public final int nextbufferhead(final int size) {
		int top = buffertop();
		return top + 1 < size ? top + 1 : out_index;
	}
	
	public final int nextbuffernext(final int size) {
		int top = buffertop();
		return top + 2 < size ? top + 2 : out_index;
	}
	
	public final int beforebufferhead() {
		int top = buffertop();
		return top > 0 ? top - 1 : out_index;
	}
	
	public final int beforebufferbefore() {
		int top = buffertop();
		return top > 1 ? top - 2 : out_index;
	}
	...
	public void Shift(int label) {
		int right = buffertop();
		m_lStack[++stack_back] = right;
		m_lCCGLabels[right] = label;
		if (right == m_nNextWord) {
			++m_nNextWord;
			ClearNext();
		} else {
			--buffer_back;
	...
	public void Swap() {
		m_lBuffer[++buffer_back] = m_lStack[stack_back - 1];
		m_lStack[stack_back - 1] = m_lStack[stack_back];
		--stack_back;
